...
//	HIDP_CAPS		Capabilities;   08Mar2005 - Capabilities call fails on WinXP
	...
	DWORD			dwError;
	HANDLE			hEventObject;
	HANDLE			wrhEventObject;
	HANDLE			hDevInfo;
	GUID			HidGuid;
	OVERLAPPED		HIDOverlapped;
	OVERLAPPED		HIDWrOverlapped;
	char			InputReport[65];
	ULONG			Length;
...
bool CUsbhidioc::FindTheHID(int unitNumber)
{
	...
	const unsigned int pkob_ProductID = 0x8107;	// PKOB3
	...
	*/
				...
				if (Result != 0)
				{
					...
					if (Attributes.VendorID == VendorID)
					{
						if (Attributes.ProductID == pickit2_ProductID 
							|| Attributes.ProductID == pickit3_ProductID
							|| Attributes.ProductID == pkob_ProductID)
						{
							...
							if (unitNumber == unitIndex)
							{
								...
								if (Attributes.ProductID == pickit2_ProductID)
									m_type = Pickit2;
								else if (Attributes.ProductID == pickit3_ProductID)
									m_type = Pickit3;
								else
									m_type = pkob;
	...
	while ((LastDevice == FALSE) && (DeviceDetected == false) && (MemberIndex < 64)); // check 64 devices max, increased from 24 on 11.2.2023
...
/*
	...
	*/
    ...
    if (DeviceDetected)
    {
		...
		&BytesWritten,
		(LPOVERLAPPED) &HIDWrOverlapped);
		// NULL);

		// Begin overlapped write additions
	...
	//return Result;
		...
		// End overlapped write additions
